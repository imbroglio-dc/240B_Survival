...
full_data <- read_rds("../data/imputed_data.RDS") %>% filter(T_return < 1000, T_ltfu < 1000)
...
We're interested in comparing the impact of tracing assignment on (1) time to return to care and (2) duration of retention in care (assuming tracing has no effect on return to care)

## Progress - Data Processing
- Conducted data cleaning and imputation of missing values
- Pop 1: Data set consisting of `r nrow(full_data)` entries
    - `r nrow(txt_data)` individuals assigned to treatment
    - `r nrow(control_data)` individuals assigned to control
- Pop 2: Data set consisting of `r nrow(returned)` entries
    - `r nrow(txt_returned)` individuals assigned to treatment
    - `r nrow(control_returned)` individuals assigned to control
